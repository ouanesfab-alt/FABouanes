...
# Bypasser CSRF et authentification
...
res = client.get("/catalog/new?kind=finished", headers={"Accept": "text/html"})
...
res = client.post("/catalog/new", data=form_preset, follow_redirects=False)
...
res = client.post("/catalog/new", data=form_custom, follow_redirects=False)
assert res.status_code == 303 or res.status_code == 200
...
res_edit_page = client.get(f"/products/{product_id}/edit", headers={"Accept": "text/html"})
...
res_edit_post = client.post(f"/products/{product_id}/edit", data=form_edit, follow_redirects=False)
