...
@async_compat
async def create_production_from_form(form):
    ...
    product = await query_db_async("SELECT * FROM finished_products WHERE id = %s", (finished_id,), one=True)
    if not product:
    ...
    recipe_id = None
    with db_transaction():
        batch_id = await execute_db_async(
        ...
        for line in recipe_lines:
            ...
            await apply_raw_material_consumption(line["material"], line["qty"], "production", batch_id, "create_production")
        await apply_finished_production(product, output_qty, total_cost, batch_id)
        if save_recipe_flag:
    ...
    if recipe_id:
        recipe = await query_db_async("SELECT * FROM saved_recipes WHERE id = %s", (recipe_id,), one=True)
...
@async_compat
async def delete_production_by_id(batch_id: int) -> bool:
    before = await query_db_async("SELECT * FROM production_batches WHERE id = %s", (batch_id,), one=True)
    ok = await reverse_production(batch_id)
    if ok:
