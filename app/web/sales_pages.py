...
async def new_sale_submit(request: Request):
    denied = require_permission(request, PERMISSION_OPERATIONS_WRITE)
    if denied:
        return denied
    await csrf_protect(request)
...
async def edit_sale_document_page(request: Request, document_id: int):
    denied = require_permission(request, PERMISSION_OPERATIONS_WRITE)
    if denied:
        return denied
...
async def edit_sale_document_submit(request: Request, document_id: int):
    denied = require_permission(request, PERMISSION_OPERATIONS_WRITE)
    if denied:
        return denied
    await csrf_protect(request)
...
async def edit_sale_page(request: Request, kind: str, row_id: int):
    denied = require_permission(request, PERMISSION_OPERATIONS_WRITE)
    if denied:
        return denied
...
async def edit_sale_submit(request: Request, kind: str, row_id: int):
    denied = require_permission(request, PERMISSION_OPERATIONS_WRITE)
    if denied:
        return denied
    await csrf_protect(request)
...
async def delete_sale(request: Request, kind: str, row_id: int):
