...
from app.api.v1._common import json_response, query_db
...
async def api_get_alerts(request: Request):
    await asyncio.to_thread(require_api_user, request, PERMISSION_CATALOG_READ)
...
async def api_acknowledge_alert(request: Request, alert_id: int):
    await asyncio.to_thread(require_api_user, request, PERMISSION_CATALOG_WRITE)
