...
from app.modules.sales.infrastructure.repository import SaleRepository, RawSaleRepository, SaleDocumentRepository
from app.core.request_state import get_state_value
from app.modules.sales.api.schemas import SaleFormSchema
from app.modules.sales.domain.services import SalesValidator
...
class SalesCommands:
    ...
    async def edit_sale_document_from_form(self, document_id: int, schema: SaleFormSchema) -> dict:
        from app.modules.sales.application.queries import SalesQueries
    ...
    async def edit_single_sale_from_form(self, kind: str, row_id: int, schema: SaleFormSchema) -> dict:
        ...
        if not lines:
            raise ValidationError("Une vente nécessite au moins un article.")

        use_document = len(lines) > 1 or before.get("document_id") is not None
