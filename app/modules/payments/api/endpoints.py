...
from app.api.v1._common import json_response, payment_payload, add_cache_headers
...
async def api_create_payment(request: Request, payload: PaymentFormSchema, db: AsyncSession = Depends(get_async_session)):
    await asyncio.to_thread(require_api_user, request, PERMISSION_OPERATIONS_WRITE)
    service = PaymentsService(db)
    try:
        payment_id, payment_type = await service.create_payment_from_form(payload)
...
async def api_update_payment(request: Request, payment_id: int, payload: PaymentFormSchema, db: AsyncSession = Depends(get_async_session)):
    await asyncio.to_thread(require_api_user, request, PERMISSION_OPERATIONS_WRITE)
    payment = await payment_payload(payment_id, db=db)
    if not payment:
        api_error("not_found", "Paiement introuvable.", 404)
    ...
    service = PaymentsService(db)
    try:
        await service.edit_payment_from_form(payment_id, payload)
    except ValueError as e:
        api_error("invalid_value", str(e), 400)
...
async def api_delete_payment(request: Request, payment_id: int, db: AsyncSession = Depends(get_async_session)):
    ...
    service = PaymentsService(db)
    success = await service.delete_payment_by_id(payment_id)
    if not success:
