...
from app.api.v1._common import json_response, production_payload, payload_to_form_data, add_cache_headers
...
from app.modules.catalog.infrastructure.repository import list_production_batches, list_recipes
...
async def api_create_production_batch(request: Request, payload: ProductionCreateSchema, db: AsyncSession = Depends(get_async_session)):
    ...
    try:
        result = await service.create_production_from_form(form_data)
...
async def api_delete_production_batch(request: Request, batch_id: int, db: AsyncSession = Depends(get_async_session)):
    ...
    if not batch:
        api_error("not_found", "Production introuvable.", 404)
    service = ProductionService(db)
    success = await service.delete_production_by_id(batch_id)
    if not success:
