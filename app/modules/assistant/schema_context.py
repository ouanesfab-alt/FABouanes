...
ACTION_GUIDE = (
    ...
    "• CA du mois : SELECT SUM(total) AS CA FROM sales WHERE sale_date >= date('now', 'start of month')\n"
    "• Bénéfice net du mois : SELECT SUM(profit_amount) AS benefice FROM sales WHERE sale_date >= date('now', 'start of month')\n"
    "• Achats du mois : SELECT SUM(total) AS total_achats FROM purchases WHERE purchase_date >= date('now', 'start of month')\n"
    "• Dépenses du mois : SELECT SUM(amount) AS total_depenses FROM expenses WHERE date >= date('now', 'start of month')\n"
    ...
    "• Historique financier mensuel SQLite (CA, Achats, Dépenses) :\n"
    "  SELECT strftime('%Y-%m', sale_date) AS mois, SUM(total) AS ca, SUM(profit_amount) AS benefice\n"
    "  FROM sales GROUP BY mois ORDER BY mois DESC LIMIT 12\n"
    "\n"
    "=== MOUVEMENTS DE STOCK ===\n"
    ...
    "  FROM expenses WHERE date >= date('now', 'start of month') GROUP BY category ORDER BY total DESC\n"
