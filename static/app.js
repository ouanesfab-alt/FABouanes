...
(function(){
  ...
  function setupGrid(table){
    ...
    const scrollDiv=document.createElement('div'); scrollDiv.className='table-scroll table-responsive';
    table.classList.add('table-sticky');
    table.parentNode.insertBefore(wrap,table);
    wrap.appendChild(bar); wrap.appendChild(scrollDiv); scrollDiv.appendChild(table);
    ...
    Array.from(table.querySelectorAll('thead th')).forEach((th,idx)=>{
      ...
      th.style.cursor='pointer';th.title='Trier';
      th.addEventListener('click',()=>{
        const next=th.dataset.sortDir==='asc'?'desc':'asc';
        table.querySelectorAll('thead th').forEach(h=>{delete h.dataset.sortDir;h.textContent=h.textContent.replace(/ [↑↓]$/,'');});
        th.dataset.sortDir=next;
        th.textContent=th.textContent+' '+(next==='asc'?'↑':'↓');
