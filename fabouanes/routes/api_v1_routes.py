...
from fabouanes.domain.exceptions import ConflictError, FabouanesError, NotFoundError, ValidationError
...
def _api_error_from_exception(exc: FabouanesError):
    ...
    return _api_error("application_error", str(exc), 400)
...
def _client_balance_sql(alias: str = "c") -> str:
    return f"""
...
def _client_total_sales_sql(alias: str = "c") -> str:
    return f"""
...
def _client_total_payments_sql(alias: str = "c") -> str:
...
def _client_payload(client_id: int):
    row = query_db(
        f"""
        SELECT c.*,
               {_client_balance_sql("c")} AS current_balance,
               {_client_balance_sql("c")} AS current_debt,
               {_client_total_sales_sql("c")} AS total_sales,
               {_client_total_payments_sql("c")} AS total_payments
        FROM clients c
        WHERE c.id = ?
        """,
        (client_id,),
